    ...
    $('div.like').on('click', function (e) {
        $.colorbox({
            ...
            'html': '<div class="dialoug alert %type%"><h2>%title%</h2>%message%</div>'
                .replace('%title%', Translator.trans('wall.like.popup.header'))
...
$(function () {
    ...
    $('div.wall-post a[rel="wall-delete"]').on('click', function (e) {
        e.preventDefault();
        $a = $(this);
        dialoug.confirm(Translator.trans('notice'), Translator.trans('wall.delete.entry.confirm'), function (choice) {
            if (choice == 'ok') {
                $.ajax({
                    url: $a.attr('href'),
                    dataType: 'json',
