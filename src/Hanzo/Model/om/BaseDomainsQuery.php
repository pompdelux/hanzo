...
abstract class BaseDomainsQuery extends ModelCriteria
{
	 ...
	 *
	 * @return    Domains|array|mixed the result, formatted by the current formatter
	 */
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new Domains();
			$obj->hydrate($row);
			DomainsPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
	 ...
	 *
	 * @param     DomainsSettings $domainsSettings  the related object to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    DomainsQuery The current query, for fluid interface
	 */
	public function filterByDomainsSettings($domainsSettings, $comparison = null)
	{
		...
		} elseif ($domainsSettings instanceof PropelCollection) {
			return $this
