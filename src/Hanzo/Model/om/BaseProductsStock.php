...
abstract class BaseProductsStock extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'Hanzo\\Model\\ProductsStockPeer';
	 ...
	 * Note: this column has a database default value of: '1969-11-30'
	 * @var        string
	 */
	...
	{
		$this->available_from = '1969-11-30';
	}

	/**
	 * Initializes internal state of BaseProductsStock object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     ProductsStock The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = ProductsStockPeer::ID;
		}
	...
	public function setProductsId($v)
	{
		...
		if ($this->products_id !== $v) {
			$this->products_id = $v;
			$this->modifiedColumns[] = ProductsStockPeer::PRODUCTS_ID;
		}
	...
	public function setQuantity($v)
	{
		...
		if ($this->quantity !== $v) {
			$this->quantity = $v;
			$this->modifiedColumns[] = ProductsStockPeer::QUANTITY;
		}
	...
	public function setAvailableFrom($v)
	{
		...
		if ($this->available_from !== null || $dt !== null) {
			...
			if ( ($currentDateAsString !== $newDateAsString) // normalized values don't match
				|| ($dt->format('Y-m-d') === '1969-11-30') // or the entered value matches the default
				 ) {
	...
	{
			if ($this->available_from !== '1969-11-30') {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->available_from = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			throw new PropelException("Error populating ProductsStock object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ProductsStockPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = ProductsStockQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				ProductsStockPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		$sql = sprintf(
			'INSERT INTO `products_stock` (%s) VALUES (%s)',
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ProductsStockPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ProductsStockPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['ProductsStock'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['ProductsStock'][$this->getPrimaryKey()] = true;
		$keys = ProductsStockPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getProductsId(),
			$keys[2] => $this->getQuantity(),
			$keys[3] => $this->getAvailableFrom(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setAvailableFrom($arr[$keys[3]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ProductsStockPeer::ID)) $criteria->add(ProductsStockPeer::ID, $this->id);
		if ($this->isColumnModified(ProductsStockPeer::PRODUCTS_ID)) $criteria->add(ProductsStockPeer::PRODUCTS_ID, $this->products_id);
		if ($this->isColumnModified(ProductsStockPeer::QUANTITY)) $criteria->add(ProductsStockPeer::QUANTITY, $this->quantity);
		if ($this->isColumnModified(ProductsStockPeer::AVAILABLE_FROM)) $criteria->add(ProductsStockPeer::AVAILABLE_FROM, $this->available_from);
	...
	{
		$criteria = new Criteria(ProductsStockPeer::DATABASE_NAME);
		$criteria->add(ProductsStockPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ProductsStock (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ProductsStock Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     ProductsStockPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new ProductsStockPeer();
		}
		return self::$peer;
	}
	...
	public function getProducts(PropelPDO $con = null)
	{
		if ($this->aProducts === null && ($this->products_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aProducts->addProductsStocks($this);
			 */
		}
	...
	{
		return (string) $this->exportTo(ProductsStockPeer::DEFAULT_STRING_FORMAT);
	}
