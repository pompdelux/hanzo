...
namespace Hanzo\Bundle\RedisBundle\DependencyInjection;
...
class RedisExtension extends Extension
{
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        foreach ($config['class'] as $name => $settings) {
            $def = new Definition($container->getParameter('redis.class'));
            $def->setPublic(true);
            ...
            $def->addMethodCall('setLogger', [new Reference('redis.logger')]);
            $container->setDefinition(sprintf('redis.%s', $name), $def);
        }
    }
}
